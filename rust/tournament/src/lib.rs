...
pub fn tally(match_results: &str) -> String {
    ...
    match_results.lines().for_each(|line| {
        ...
        scores.entry(home.into()).or_insert(Team::new(home.into())).add_match(&result);
        scores.entry(away.into()).or_insert(Team::new(away.into())).add_match(&result.reverse());
    });
    ...
    vec![String::from(HEADER)].into_iter().chain(score_values.into_iter().map(|t| t.into())).collect::<Vec<String>>().join("\n")
}
